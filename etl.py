...
def process_log_data(spark, input_data, output_data):
    ...
    time_table = df.select(col('Timestamp').alias('start_time'), hour(col('Timestamp')).alias("hour"), dayofmonth(col('Timestamp')).alias("day"), \
                           ...
                           year(col('Timestamp')).alias("year"), dayofweek(col('Timestamp')).alias("weekday"))
    ...
    song_data_df = spark.read.parquet(song_data_parq_path).select("song_id","title","artist_id","duration","artist_name")
